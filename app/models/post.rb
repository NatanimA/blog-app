class Post < ApplicationRecord
    ...
    scope :find_comments_amount, ->(id) { select(:comments_counter).where(:id=> id)}

    #This scope will find the current amount of likes a post have by the given Post ID.
    scope :find_likes_amount, ->(id) { select(:likes_counter).where(:id=> id)}
    ...
    def self.update_likes_counter(id)
        ...
        Post.where(:id => id).update(:likes_counter => count)
    end
    ...
    def self.update_comments_counter(id)
        ...
        Post.where(:id => id).update(:comments_counter => count)
    end

    #Here we can reuse this code to find Posts that belongs to a user
    scope :posts_by_author_id, ->(author) { where(:author => author).order(:created_at => "DESC").first(3)}
