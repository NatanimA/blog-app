...
Rails.describe 'Blog App', type: :system do
  ...
  context "#User Page Interaction" do
    ...
    it "Should Display the Bio of the user" do
      visit user_path(@user.id)
      expect(page).to have_css(".user-bio-content",text:"#{@user.bio}")
    end

    it "Should display the first 3 recent posts" do
      visit user_path(@user.id)
        expect(page).to have_css('.posts-content',text:"#{@post1.text}")
        expect(page).to have_css('.posts-content',text:"#{@post2.text}")
        expect(page).to have_css('.posts-content',text:"#{@post3.text}")
    end
  ...
  context "#User post index page:" do
    ...
    it "Should see the number of posts the user has written." do
      visit user_posts_path(@user.id)
      expect(page).to have_css(".num-posts", text: "#{@user.posts_counter}")
    end

    it "Should see a post's title." do
      visit user_posts_path(@user.id)
      expect(page).to have_css(".post-title-header", text: "#{@post1.title}")
      expect(page).to have_css(".post-title-header", text: "#{@post2.title}")
      expect(page).to have_css(".post-title-header", text: "#{@post3.title}")
    end
    ...
    it "Should see the first comments on a post" do
      visit user_posts_path(@user.id)
      expect(page).to have_css(".comments-content", text: "#{@comment.text}")
    end
  ...
  context "#Post show page:" do
    ...
    it "can see who wrote the post." do
      visit user_post_path(@user.id,@post1.id)
      expect(page).to have_css('.poster-user-name', text: "#{@user.name}")
    end
    ...
    it "can see the post body." do
      visit user_post_path(@user.id,@post1.id)
      expect(page).to have_css('.post-content-article', text: "#{@post1.text}")
    end
